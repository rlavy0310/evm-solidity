...
contract USP is IERC20 {
    ...
    function loansRate(uint256 tokens)public view returns(uint256){
        uint exchange_amount = getOSMValue(unit) * tokens /getOSMValue(usp) *10000 /15000;
    ...
    function loans()public payable returns(uint){
        require (msg.value <= (usp_bank_balances *(10000 - 1000) *15000 *getOSMValue(usp) /getOSMValue(unit) /100000000), "loan overflow");
        ...
        acc.debt = loanAmount *(10000 + 5) /10000; 
        acc.liquidationState = 0;
        loanAccount[msg.sender].push(acc);
        usp_bank_balances -= loanAmount;
        user_balances[msg.sender] += loanAmount;
        loansTotalInterest += (loanAmount *5 /10000); 
        _totalSupply += (loanAmount *5 /10000);
        interestMint += (loanAmount *5 /10000);
        ...
        for(uint a=0; a<loanUsers.length; a++){
            for(uint b=0; b<loanAccount[loanUsers[a]].length; b++){
                ...
                if(loanAccount[loanUsers[a]][b].liquidationState != 2){
                    while(k>0){
                        loansTotalInterest += (loanAccount[loanUsers[a]][b].debt *5 /10000);
                        interestMint += (loanAccount[loanUsers[a]][b].debt *5 /10000);
                        _totalSupply += (loanAccount[loanUsers[a]][b].debt *5 /10000);
                        loanAccount[loanUsers[a]][b].debt += (loanAccount[loanUsers[a]][b].debt *5 /10000);
                        k--;
                    }
                }
            }
        ...
        for(uint m=0; m < loanUsers.length; m++){
            for(uint n=0; n < loanAccount[loanUsers[m]].length; n++){
                if((loanAccount[loanUsers[m]][n]).liquidationState != 2){
                    if(getOSMValue(unit) * (loanAccount[loanUsers[m]][n]).mortgage <= getOSMValue(usp) * (loanAccount[loanUsers[m]][n]).debt *12500 /10000){
                        if(getOSMValue(unit) * (loanAccount[loanUsers[m]][n]).mortgage >= getOSMValue(usp) * (loanAccount[loanUsers[m]][n]).debt *11000 /10000){
    ...
    function addLoans(uint number)public payable returns(bool){
        ...
        if(loanAccount[msg.sender][number].mortgage * getOSMValue(unit) *10000 /12500 >= loanAccount[msg.sender][number].debt * getOSMValue(usp)){
